...
model = 'StyleDiffGAN3_G'
modelKwargs = [{'tgt_train_ratio':1, 'get_pretrained_D':1}]

replay = 'no-replay-fs' ## replay type
...
note = ['diffgan-sp3_{}'.format(replay)] ## model

## pretrained_weights
base_path = 'save7/cl/baseline/mixstage_baseline/{}/"$(ls save7/cl/baseline/mixstage_baseline/{} | grep weights | tail -1)"'.format('_'.join(src_sp), '_'.join(src_sp))
