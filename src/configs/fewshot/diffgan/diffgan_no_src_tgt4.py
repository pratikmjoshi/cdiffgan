...
Ks = [1, 10, 30, 50, 64]
for k in Ks:
  modelKwargs.append({'tgt_train_ratio':1, 'get_pretrained_D':1, 'fewshot_k':k})
...
Ks_kshot_sp_seeds_save_dir = []
for modelKwarg in modelKwargs:
  for k_shot in k_shots:
    for sp in speakers:
      for sd in fewshot_seeds:
        try:
          Ks_kshot_sp_seeds_save_dir.append((pretrained_model_weights[speakers_rev[sp[0][0]]], convert2str(modelKwarg), k_shot, convert2str(sp), sd, 'save6/fewshot/models/{}/{}/psi_k_{}/{}/{}_{}/{}'.format(note.split('_')[0], note, modelKwarg['fewshot_k'], k_shot, sp[0][0], sp[1][0], sd)))
        except:
          pdb.set_trace()
optim = 'AdamW'
lr = 0.0001
...
conv_config={
  ...
  ('pretrained_model_weights', 'modelKwargs', 'k_shot', 'speaker', 'fewshot_seed', 'save_dir'):Ks_kshot_sp_seeds_save_dir,
  'paradigm':[paradigm],
