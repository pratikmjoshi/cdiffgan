...
for s, e in zip(start, end):
  modelKwargs.append({'tgt_train_ratio':1, 'get_pretrained_D':1, 'fewshot_LAYER_START':s, 'fewshot_LAYER_END':e})

note = 'diffgan_{}'.format(paradigm)
...
layers_kshot_sp_seeds_save_dir = []
for modelKwarg in modelKwargs:
  for k_shot in k_shots:
    for sp in speakers:
      for sd in fewshot_seeds:
        try:
          layers_kshot_sp_seeds_save_dir.append((pretrained_model_weights[speakers_rev[sp[0][0]]], convert2str(modelKwarg), k_shot, convert2str(sp), sd, 'save6/fewshot/models/{}/{}/layers{}_{}/{}/{}_{}/{}'.format(note.split('_')[0], note, modelKwarg['fewshot_LAYER_START'], modelKwarg['fewshot_LAYER_END'], k_shot, sp[0][0], sp[1][0], sd)))
        except:
          pdb.set_trace()
optim = 'AdamW'
lr = 0.0001
...
conv_config={
  ...
  ('pretrained_model_weights', 'modelKwargs', 'k_shot', 'speaker', 'fewshot_seed', 'save_dir'):layers_kshot_sp_seeds_save_dir,
  'paradigm':[paradigm],
