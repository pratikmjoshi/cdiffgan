## Quantize on all time scales with a total of ~512 embeddings with residual_mask=[0, 0, 1, 1, 1]
import json
import itertools
def convert2str(x):
  x = json.dumps(x)
...
model = 'JointLateClusterVQ4_G'
modelKwargs = [{'num_embeddings':85, 'residual_mask':[0, 0, 1, 1, 1]}]
note = 'vqvae_timescale5'
...
modelKwargs = [convert2str(m) for m in modelKwargs]
