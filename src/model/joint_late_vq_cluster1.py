...
JointLateClusterSoftVQ1_D = Speech2Gesture_D

class JointLateClusterSoftVQ1_G(nn.Module):
  '''
...
#    self.text_encoder = TextEncoder1D(output_feats = time_steps, p=p)
  ...
  def forward(self, x, y, time_steps=None, **kwargs):
    ...
    if torch.rand(1).item() > self.thresh.step(self.training) and self.training:
      ...
      with torch.no_grad(): ## run to find pseudo cluster labels
        x_gt_cap = self.pose_encoder(y, time_steps)
        x_gt_cap = self.unet(x_gt_cap)
        _, labels_score_gt_cap = self.vq(x_gt_cap)
        labels_gt_cap = labels_score_gt_cap.min(dim=-1)[1]
      ...
      x = self.unet(x)
      x, labels_score = self.vq(x)
      internal_losses.append(self.classify_loss(labels_score.reshape(-1, labels_score.shape[-1]), labels_gt_cap.reshape(-1)))
