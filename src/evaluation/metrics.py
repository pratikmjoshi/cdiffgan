...
def get_model(path2weights):
  ...
  book = BookKeeper(args, [], args_dict_update = {'load_data':0, 'pretrained_model':1, 'sample_all_styles':0, 'mix':0, 'optim_separate':None, 'path2data':args.path2data, 'replay':None, 'fewshot_seed':None})
...
class Expressiveness():
  ...
  def window_smoothing(self, x, k=5):
    ...
    weight = torch.ones(x.shape[-2], 1, k).float()/k
    padding = int((k-1)/2)
    with torch.no_grad():
...
class IOU():
  def __init__(self, num_clusters=8, cluster_p=None):
    self.average_meter = AverageMeter('cluster_IOU')
    self.num_clusters = num_clusters
    self.cluster_z = OnlineKMeans(num_clusters)
    self.cluster_p = cluster_p

  def match(self, x, Is, Js):
    sums = torch.zeros(x.shape[0]) - np.inf
    BIs = []
    bis = torch.zeros(x.shape[0])
    if len(Is) == 1:
      bi, bj = list(Is)[0], list(Js)[0]
      return x[bi, bj], bi, []
    for i in range(x.shape[0]):
      if i in Is:
        ...
        j = min(Js)
        Js.remove(j)
        sum_subset, bi, BI = self.match(x, Is, Js)
        BIs.append(BI)
        bis[i] = bi
        try:
          sums[i] = x[i, j] + sum_subset 
        except:
          pdb.set_trace()
        Is.add(i) # backtracking
        Js.add(j)
      else:
        BIs.append([])
    sum_max = sums.max(dim=0)
    idx = sum_max.indices.item()
    BBI = [bis[idx].int().item()] + BIs[idx]
    return sum_max.values, idx, BBI

  def cluster_match(self, mat):
    max_score, bi1, b1 = self.match(mat, set(range(mat.shape[0])), set(range(mat.shape[0])))
    return max_score, list(zip([bi1]+b1, range(mat.shape[-1])))
    
  def cost(self, a, b):
    C = torch.zeros(self.num_clusters, self.num_clusters)
    for i in range(self.num_clusters):
      for j in range(self.num_clusters):
        C[i,j] = (((a == i).int() + (b == j).int()) == 2).sum()
    return C
    
  def __call__(self, z, p, mask_idx=[0, 7, 8, 9]):
    p_labels = self.cluster_p(p)
    z_labels = self.cluster_z(z)
    C = self.cost(z_labels, p_labels)
    n = z.shape[0]*z.shape[1]
    self.average_meter.update(C/n, n=n)

  def reset(self, **kwargs):
    self.average_meter.reset()
    self.cluster_z.reset(description=kwargs['description'])

  def get_averages(self, desc):
    score = 0
    if len(self.average_meter.avg.shape) > 0:
      score = self.cluster_match(self.average_meter.avg)[0].item()
    return {'{}_cluster_IOU'.format(desc):score}

class KNNMonitor():
  def __init__(self, num_clusters=8):
    self.average_meter = AverageMeter('knn_monitor')
  ...
  def knn_predict(self, feature):
    ...
    one_hot_label = torch.zeros(feature.size(0) * self.knn_k, self.num_clusters, device=sim_labels.device)
    # [B*K, C]
    one_hot_label = one_hot_label.scatter(dim=-1, index=sim_labels.view(-1, 1), value=1.0)
    # weighted score ---> [B, C]
    pred_scores = torch.sum(one_hot_label.view(feature.size(0), -1, self.num_clusters) * sim_weight.unsqueeze(dim=-1), dim=1)

    pred_labels = pred_scores.argsort(dim=-1, descending=True)
    return pred_labels

  def __call__(self, feature, target, mask_idx=[0, 7, 8, 9]):
    pred_labels = self.knn_predict(feature)
    sums = (pred_labels[:, 0] == target).float().sum()
    count = pred_labels.shape[0]
    self.average_meter.update(sums/count, n=count)

  def update(self, feature, label):
    # self.feature_bank.append(feature.transpose(-2, -1))
    # self.feature_labels.append(label)
    if self.feature_bank is None:
      self.feature_bank = feature.transpose(-2, -1)
      self.feature_labels = label
    else:
      self.feature_bank = torch.cat([self.feature_bank, feature.transpose(-2, -1)], dim=-1)
      self.feature_labels = torch.cat([self.feature_labels, label], dim=0)
      if self.feature_bank.shape[-1] > self.max_count:
        idx = torch.randperm(self.feature_bank.shape[-1])[:self.max_count]
        self.feature_bank = self.feature_bank[..., idx]
        self.feature_labels = self.feature_labels[idx]
  ...
  def reset(self, **kwargs):
    ...
    #     self.feature_bank = torch.cat(self.feature_bank, dim=-1)
    #     self.feature_labels = torch.cat(self.feature_labels, dim=0)

  def get_averages(self, desc):
    return {'{}_knn_monitor'.format(desc):self.average_meter.avg.item()}
