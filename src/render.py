...
from pycasper.argsUtils import get_args_update_dict
...
def wandb_videos(desc, speaker, filenames, intervals, k=3):
  np.random.seed(11212)
  perm = np.random.permutation(len(filenames))[:k]
  for i in perm:
    wandb.log({'video_{}_{}_{}'.format(desc, speaker, intervals[i]):wandb.Video(filenames[i],
                                                                                fps=15,
                                                                                format='mp4')})
...
def render(args, exp_num):
  ...
  args_dict_update = get_args_update_dict(args)
  args_dict_update.update({'render':args.render})
  ...
  ## Wandb
  if args.wandb:
    wandb.init(dir=args.wandb_dir,
               config=args.__dict__,
               name=book.name.name,
               project=args.wandb_project,
               group='-'.join(args.speaker),
               job_type='render')  
  ...
  for desc in ['test']:#, 'train']:
    for spk in speaker:
      for k_dirname in keypoints_dirnames:
        ...
        filenames = save_animation(y_animates, intervals, dir_name, desc, data, start, subname=subname1, text=texts, output_modalities=output_modality, mask=mask)
        ...
        ## Wandb
        if args.wandb:
          wandb_videos(desc, spk, filenames, intervals)
