...
class Skeleton2D(Modality):
  def __init__(self, path2data='../dataset/groot/data/speech2gesture_data',
    ...
    try:
        ...
        pass
    try:
        self.df = self.df.append(pd.read_csv((Path(self.path2data)/'d2m_intervals_df.csv').as_posix())) ## file dance dataset
    except:
        pass
    try:
        self.df = self.df.append(pd.read_csv((Path(self.path2data)/'ted_intervals_df.csv').as_posix())) ## file with ted dataset    
    except:
        pass
