...
class DummyData(Dataset):
  ...
  def __getitem__(self, idx):
    ...
    return {variable:self.data[variable][idx].to(torch.float) for variable in self.variable_list}
...
class Data(Modality):
  ...
  def __init__(self, path2data, speaker,
    ...
    self.df = self.df.append(pd.read_csv((Path(self.path2data)/'d2m_intervals_df.csv').as_posix())) ## with dancing2music dataset 
    self.df = self.df.append(pd.read_csv((Path(self.path2data)/'ted_intervals_df.csv').as_posix())) ## with dancing2music dataset   
...
#      self.df = self.get_df_subset('speaker', speaker)
  ...
  def tdt_split(self):
    ...
    train_minidata_list = self.get_minidata_list(train_intervals)
    dataset_train = ConcatDatasetIndex(train_minidata_list)
    if self.num_training_sample is not None:
      self.subset_idx = torch.randperm(len(dataset_train))[:self.num_training_sample]
      dataset_train = ConcatDatasetIndex(train_minidata_list, self.subset_idx)
    else:
  ...
  def update_dataloaders(self, time, window_hop):
    ...
    if self.fewshot_seed is not None: ## force batch size to size of k_shot examples
      train_dataLoader_kwargs['batch_size'] = self.k_shot * self.p_avg

    if self.fewshot_seed is not None and self.replay is not None: ## replay for CL
      ## __len__ property of the sampler should give the correct value
      train_dataLoader_kwargs['batch_size'] = len(self.train_sampler)
      # if self.replay == 'random-fs':
      #   train_dataLoader_kwargs['batch_size'] = self.k_shot * len(self.speaker)
      # elif self.replay == 'random-full':
      #   train_dataLoader_kwargs['batch_size'] = len(self.train_sampler)
      # elif self.replay == 'no-replay-fs':
      #   train_dataLoader_kwargs['batch_size'] = self.k_shot
      # elif self.replay == 'no-replay-full':
      #   train_dataLoader_kwargs['batch_size'] = self.k_shot        ##TODO
      # elif self.replay == 'importance-fs':
      #   pass
      # elif self.replay == 'pseudo-src': ## keep source input
      #   pass
      # elif self.replay == 'pseudo-tgt': ## use target input
      #   pass
      # else:
      #   assert 0, 'self.replay == {} is not supported'.format(self.replay)        
      
    self.train = DataLoader(ConcatDatasetIndex(self.datasets['train'].datasets, self.subset_idx), sampler=self.train_sampler, **train_dataLoader_kwargs)
  ...
  def get_multi_class_fewshot_sampler(self, dataset, intervals_dict, num_samples, seed, class_list=None):
    class_count = []
    interval_offset = 0
    for tup in intervals_dict:
      count = 0
    ...
    return MultiClassFewshotSampler(class_count, num_samples, seed=seed, replacement=True, class_list=class_list)
  ...
  def update_intervals(self, train_intervals, dev_intervals, test_intervals):
    ...
    if self.sample_all_styles != 0:
      ...
      train_intervals_dict, train_intervals = self.order_intervals(train_intervals)
    elif self.fewshot_seed is not None and self.replay is not None: ## replay for CL
      train_intervals_dict, train_intervals = self.order_intervals(train_intervals)
    else:
  ...
  def get_quantile_sample(self, data, q):
    ...
    if kind == 'above':
      v0 = np.quantile(np.array(samples, dtype=np.float32), q)
      print('above {} percentile'.format(v0))
    elif kind == 'tail':
      v0 = [np.quantile(np.array(samples, dtype=np.float32), q[0]), np.quantile(np.array(samples, dtype=np.float32), q[1])]
  ...
  def get_train_sampler(self, dataset_train, train_intervals_dict):
    ...
    if self.style_iters > 0 and self.sample_all_styles == 0:
      ...
      train_sampler.weights = train_sampler.weights.float()
    ...
    elif self.fewshot_seed is not None and self.replay is not None:
      ## typically works with multiple styles in the same model.
      if self.replay == 'random-fs':
        train_sampler = self.get_multi_class_fewshot_sampler(dataset_train, train_intervals_dict, self.k_shot, self.fewshot_seed)
      elif self.replay == 'random-full':
        train_sampler = self.get_multi_class_fewshot_sampler(dataset_train, train_intervals_dict, None, self.fewshot_seed)
      elif self.replay == 'no-replay-fs': ## no replay from the past and fewshot examples from current
        train_sampler = self.get_multi_class_fewshot_sampler(dataset_train, train_intervals_dict, self.k_shot, self.fewshot_seed,
                                                             class_list = [-1])
      elif self.replay == 'no-replay-full': ## no replay from the past and all examples from current
        train_sampler = self.get_multi_class_fewshot_sampler(dataset_train, train_intervals_dict, None, self.fewshot_seed,
                                                             class_list = [-1])
      elif self.replay == 'importance-fs':
        pass
      elif self.replay == 'pseudo-fs-src': ## keep source input
        train_sampler = self.get_multi_class_fewshot_sampler(dataset_train, train_intervals_dict, self.k_shot, self.fewshot_seed)
      elif self.replay == 'pseudo-fs-tgt': ## use target input
        train_sampler = self.get_multi_class_fewshot_sampler(dataset_train, train_intervals_dict, self.k_shot, self.fewshot_seed,
                                                             class_list = [-1])
      elif self.replay == 'pseudo-align-fs-src': ## keep source input
        train_sampler = self.get_multi_class_fewshot_sampler(dataset_train, train_intervals_dict, self.k_shot, self.fewshot_seed)
      elif self.replay == 'pseudo-align-fs-tgt': ## use target input
        train_sampler = self.get_multi_class_fewshot_sampler(dataset_train, train_intervals_dict, self.k_shot, self.fewshot_seed,
                                                             class_list = [-1])
      else:
        assert 0, 'self.replay == {} is not supported'.format(self.replay)
    elif self.fewshot_seed is not None:
      train_sampler = FixedRandomSampler(dataset_train,
                                         seed=self.fewshot_seed,
                                         replacement=True,
                                         num_samples=self.k_shot*self.p_avg)
    else:
...
class MiniData(Dataset, HDF5):
  def __init__(self, path2h5, modalities, fs_new, time, modality_classes, window_hop, style=0, repeat_text=1, text_in_modalities=False, filler=0, audio_average=0, cache=False, **kwargs):
    ...
    if not self.cache:
      self.load_data(shapes=True)
    else:
      self.load_shapes()
      self.data = None

  ## load data  
  def load_data(self, shapes=True):
    if shapes:
      self.shapes = []
    self.data = []
    for modality in self.modalities:
    ...
    if self.text_in_modalities:
      try:
        ...
        self.text_df = None
    if self.filler:
      self.stopwords = self.kwargs['stopwords']
  ...
  def _to_float32(self, item):
    for key in item:
      if isinstance(item[key], np.ndarray):
        if item[key].dtype == 'float64':
  ...
  def __getitem__(self, idx):
    if self.cache and self.data is None:
      self.load_data()
    item = {}
    ...
    for i, modality in enumerate(self.modalities):
      ...
      if self.audio_average and 'audio' in modality:
        item[modality] = self._window_average(data, start, end, interval).astype(np.float32)
      else:
        item[modality] = data[start:end:interval].astype(np.float32)
      ...
      if 'text' in modality:
        ...
        ## add filler masks
        if self.filler:
          filler = np.zeros((len(indices),), dtype=np.float32)
...
class FixedRandomSampler(Sampler):
  r"""Modification of the Random Sampler. The key difference here is that after each interation the manual seed is reset to the original value
  ...
  def __init__(self, data_source, replacement=False,
               num_samples=None, generator=None, seed=None):
  ...
  def __iter__(self):
    ...
    if self.generator is None and self.seed is None:
      generator = torch.Generator()
      generator.manual_seed(int(torch.empty((), dtype=torch.int64).random_().item()))
    elif self.generator is None and self.seed is not None:
      generator = torch.Generator()
...
class MultiClassFewshotSampler(Sampler):
  '''
  num_training_iters: for training the source and target model; number of iterations in each epoch
  --- while trianing the target model, the source speaker dataloader is just used as a proxy, but the source data is not used in the no_src_tgt paradigm
  num_iters: for number of loops in the dev and test cycle
  replay: type of replay: fewshot or full, random or importance, pseudo or real
  class_count: number of samples in each class listed sequentially 
  class_list: list of classes where the samples are taken from. If None, all classes are sampled from.
  '''
  def __init__(self, class_count, num_samples, seed, replacement=True, class_list=None):
    if num_samples is not None:
      ...
      self.num_samples_per_class = num_samples//num_classes
      self.num_samples = self.num_samples_per_class*num_classes
    else:
    ...
    if self.class_list is not None: ## only sample a few classes
      self.starts_sub = self.starts[self.class_list]
      self.ends_sub = self.ends[self.class_list]
      self.class_count_sub = self.class_count[self.class_list]
    else:
      self.starts_sub = self.starts
      self.ends_sub = self.ends
      self.class_count_sub = self.class_count
    
  def __iter__(self):
    if self.num_samples is not None: ## fewshot dataset
      generator = torch.Generator()
      generator.manual_seed(self.seed)
      if self.replacement:
        for start, end in zip(self.starts_sub, self.ends_sub):
          yield from torch.randint(start, end, size=(self.num_samples_per_class, ), dtype=torch.int64, generator=generator).tolist()
      else: 
        for start, end, n in zip(self.starts_sub, self.ends_sub, self.class_count_sub):
          yield from (torch.randperm(n, generator=generator, dtype=torch.int64)[:self.num_samples_per_class] + start).tolist()
    else: ## full dataset
      yield from torch.cat([torch.arange(start, end, dtype=torch.int64) for start, end in zip(self.starts_sub, self.ends_sub)]).tolist()
      #yield from torch.arange(self.ends[-1], dtype=torch.int64).tolist()
  ...
  def __len__(self):
    if self.num_samples is not None:
      ...
      return sum(self.class_count_sub)

  def len_prev_exp(self): ## assumes that the new experience only has one new speaker
    if self.num_samples is not None:
      return len(self) - self.num_samples_per_class
    else:
      return sum(self.class_count_sub[:-1])

class ShiftedIter():
  def __init__(self, it, shift):
    self.it = it
    self.shift = shift

  def __iter__(self):
    for idx in self.it:
      yield idx + self.shift

  def __len__(self):
    return len(self.it)
 
class ConcatDatasetIndex(ConcatDataset):
  def __init__(self, datasets, subset_idx=None):
...
class FewshotData(Modality):
  def __init__(self, path2data, speaker,
    ...
    super().__init__(path2data=path2data)
    assert isinstance(speaker[0], list), 'speaker should be a list of lists for a fewshot learning dataset'

    if speaker[0]:
      self.fewshotTrain = Data(path2data, speaker[0],
                               modalities=modalities,
                               fs_new=fs_new, time=time, 
                               split=split,
                               batch_size=batch_size, shuffle=shuffle, num_workers=num_workers,
                               window_hop=window_hop,
                               load_data=load_data,
                               ...
                               cache=cache,
                               fewshot_seed=None, ## do not need fixed random samples
                               k_shot=k_shot,
                               p_avg=p_avg)
    else:
    ...
    if speaker[1]:
      self.fewshotTest = Data(path2data, speaker[1],
                              modalities=modalities,
                              fs_new=fs_new, time=time, 
                              split=split,
                              batch_size=batch_size, shuffle=shuffle, num_workers=num_workers,
...
class ConcatIter():
  def __init__(self, iters, call_freq, num_iters=400):
    assert len(iters) == len(call_freq), 'number of iters must be the same as number of call_freq'
    self.iters = iters
  ...
  def reset(self):
    self.call_freq_counter = self.call_freq.copy()
    self.iter_idx = 0
  ...
  def __next__(self):
    if self.count_iters == self.num_iters:
      self.count_iters = 0
      raise StopIteration
    
    if self.call_freq_counter[-1] == 0:
      self.reset()
      
    if self.call_freq_counter[self.iter_idx] == 0:
      self.iter_idx += 1
    
    self.call_freq_counter[self.iter_idx] -= 1
    self.count_iters += 1
    return self.iters[self.iter_idx].__iter__().next()
