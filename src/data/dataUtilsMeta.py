...
#      self.df = self.get_df_subset('speaker', speaker)
  ...
  def get_train_sampler(self, dataset_train, train_intervals_dict):
    ...
    # elif self.num_training_sample is not None and self.num_training_iters is not None:
    #   pass ## TODO
    # elif self.num_training_sample is not None and self.weighted:
    #   ## TODO
    #   ## the training samples are not random, but the first few loaded 
    #   train_sampler = torch.utils.data.WeightedRandomSampler([1]*self.num_training_sample, self.weighted*self.batch_s                                                           ize)
...
class MetaData(Modality):
  def __init__(self, path2data, speaker,
    ...
    if speaker[0]:
      self.metaTrain = Data(path2data, speaker[0],
      ...
      warnings.warn('Meta Train not available. Skipping!!')

    if speaker[1]:
      self.metaTest = Data(path2data, speaker[1],
                           modalities=modalities,
                           fs_new=fs_new, time=time, 
                           split=split,
                           batch_size=batch_size, shuffle=shuffle, num_workers=num_workers,
      ...
      assert False, 'Meta Test not available. Exiting!!'
